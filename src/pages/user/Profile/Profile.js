...
const Profile = () => {
  ...
  const userExperienceIsLoading = useSelector(
    (state) => state.userExperience.isLoading
  );
    ...
    <MainContainer maxWidth={"xl"}>
      <UserInfo item xs={4}>
        <Box display="flex" flexDirection="column" alignItems="center">
          ...
          <Link sx={{ marginTop: "8px" }} underline="none" href={"/update"}>
            Налаштування
          </Link>
          <Coins>
            {<img src={coins} />}
          ...
          >
            {" "}
            <img src={fire} width={"60px"} height={"60px"} alt={"fire icon"} />
      ...
      <Grid item xs={8}>
        <Box display="flex" flexDirection="column" alignItems="start">
          ...
          >
            ...
            <Link underline={"none"} sx={{ fontSize: "17px" }} href={"/awards"}>
              Показати всі
            </Link>
          </Grid>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            {isLoadingAwards ? (
              ...
              <AwardList displayRandom={true} awards={awards} />
            )}
          </Box>
          <Grid
            sx={{
